...
The four Process stages share one sticky viewport only when CSS and JS both match `min-width: 1280px`, `min-height: 860px`, `pointer: fine` and `prefers-reduced-motion: no-preference`. Wheel and Page/Arrow keys advance exactly one stage while the deck is pinned; stage links scroll to the matching checkpoint and use the same opacity/translate replacement. Native scrolling is never blocked above stage 01 or below stage 04. Outside the shared guard, the same server-rendered DOM remains a complete static vertical sequence with no hidden or inert stages.
